 ...
 */
export const transformPriceLevel = (
  priceLevel?: string
): "$" | "$$" | "$$$" | "$$$$" | undefined => {
...
export const formatOpeningHours = (
  hours?: GooglePlacesApiAdvDetails["regularOpeningHours"]
): {
  displayText: string;
  isOpenNow: boolean;
} => {
  ...
  if (hours.openNow) {
    ...
    const todayPeriods =
      hours.periods?.filter((period) => period.open.day === currentDay) || [];
...
export const transformReviews = (
  reviews?: GooglePlacesApiAdvDetails["reviews"]
): AppReview[] => {
  if (!reviews) return [];
 ...
 */
export const getFirstPhotoReference = (
  photos?: GooglePlacesApiBasicDetails["photos"]
): string | null => {
 ...
 */
export const transformPhotos = (
  photos?: GooglePlacesApiBasicDetails["photos"]
): {
  photoReferences: Array<PhotoReference>;
} => {
  if (!photos) {
...
export const calculateDistanceFromUser = (
  place: GooglePlacesApiBasicDetails,
  userLatitude?: number,
  ...
  const distanceInMeters =
    parseFloat(distanceStr) * (distanceStr.includes("km") ? 1000 : 1);
...
export const transformPlaceBasicToCard = (
  ...
  const { distance, distanceInMeters } = calculateDistanceFromUser(
    place,
    userLatitude,
    userLongitude
  );
  const { photoReferences } = transformPhotos(place.photos);
...
export const transformPlacesToCards = (
  places: GooglePlacesApiBasicDetails[],
  options: PlaceTransformOptions = {}
): RestaurantCard[] => {
...
export const mergeCardWithDetails = (
  ...
  placeDetails: GooglePlacesApiAdvDetails,
  options: PlaceTransformOptions = {}
): RestaurantCard => {
  ...
  const detailedCard: RestaurantCard = { ...basicCard,
    advDetails: placeDetails,
...
export const filterCardsByPreferences = (
  ...
  return cards.filter((card) => {
    ...
    if (
      ...
      card.distanceInMeters &&
      card.distanceInMeters > preferences.maxDistance
    ) {
      return false;
    }
    ...
    if (
      ...
      card.rating &&
      card.rating < preferences.minRating
    ) {
      return false;
    }
