...
import { Mic, MicOff, Volume2, Loader2, AlertCircle, CheckCircle, Settings, RefreshCw } from 'lucide-react';
import { speechToTextService, SpeechRecognitionResult } from '@/utils/speechToText';
...
type VoiceSearchState = 'idle' | 'listening' | 'processing' | 'success' | 'error' | 'permission-needed';
type PermissionState = 'unknown' | 'granted' | 'denied' | 'prompt';

export function VoiceSearch({ onFiltersApplied, className = '' }: VoiceSearchProps) {
  ...
  useEffect(() => {
    ...
    const checkMobile = () => {
      ...
      const isCapacitorApp = !!(window as any).Capacitor || !!(window as any).cordova;
    ...
    const initializeService = async () => {
      try {
        ...
        if (typeof speechToTextService.isSupported !== 'function') {
          setInitError('Speech service not properly initialized');
          setIsSupported(false);
          return;
        }
        ...
        // Check microphone permission status quickly
        if (supported) {
          try {
            if ((window as any).Capacitor) {
              // Use fast Capacitor permission check
              const { capacitorPermissions } = await import('../../utils/capacitorPermissions');
              const permissionCheck = await capacitorPermissions.checkMicrophonePermission();
              setPermissionState(permissionCheck.granted ? 'granted' : 'prompt');
            } else {
              // Use standard permission check for web
              const permissionStatus = await speechToTextService.checkMicrophonePermission();
              setPermissionState(permissionStatus);
            }
          } catch (error) {
            console.warn('Could not check initial permission status:', error);
            setPermissionState('unknown');
          }
        }
      } catch (error) {
        console.error('VoiceSearch: Error checking speech recognition support:', error);
  ...
  const startListening = async () => {
    ...
    // Fast permission check - only show permission state if we need to request
    ...
    if (isCapacitorApp) {
      try {
        ...
        if (quickCheck.granted) {
          ...
          console.log('🚀 Permission already granted, starting speech recognition immediately');
          setState('listening');
          ...
          if (!permissionResult.granted) {
            setError(permissionResult.error || 'Microphone permission is required for voice search');
            setState('error');
        ...
        setError('Failed to handle microphone permission. Please try again.');
        setState('error');
        setPermissionState('denied');
        return;
      }
    } else {
      // Web browser - start listening directly (getUserMedia will handle permissions)
      setState('listening');
    }
    ...
    try {
      await speechToTextService.startListening(
        ...
        (error: string) => {
          ...
          if (error.includes('permission') || error.includes('denied') || error.includes('Capacitor') || error.includes('Cordova')) {
            setPermissionState('denied');
          }
        },
        () => {
  ...
  const requestPermissionAgain = async () => {
    ...
    try {
      ...
      if (isCapacitorApp) {
        // Use fast Capacitor permission manager
        const { capacitorPermissions } = await import('../../utils/capacitorPermissions');
        const permissionResult = await capacitorPermissions.requestMicrophonePermission();
        ...
        // Use standard web permission request
        const permissionResult = await speechToTextService.requestMicrophonePermission();
      ...
      console.error('Error requesting permission again:', error);
      setError('Failed to request microphone permission. Please try manually enabling it in your browser settings.');
      setState('error');
      setPermissionState('denied');
    }
  };
  ...
  const openBrowserSettings = async () => {
    ...
    if (isCapacitorApp) {
      try {
        ...
        const { capacitorPermissions } = await import('../../utils/capacitorPermissions');
        await capacitorPermissions.openSettings();
        setError('Device settings opened. Please enable microphone permissions for this app and try again.');
      } catch (error) {
        setError('To enable microphone: Go to your device Settings → Apps → [App Name] → Permissions → Microphone → Allow');
      }
    } else if (isMobile) {
      // Mobile browser instructions
      setError('To enable microphone: Go to your browser settings → Site permissions → Microphone → Allow for this site');
    } else {
      // Desktop browser instructions
      setError('Click the microphone icon in your browser\'s address bar to manage permissions');
    }
  };
        ...
        <CardContent className="p-4">
          <div className="flex items-center gap-3 text-gray-500">
            ...
            <span className="text-sm">
              Voice search is not supported in your browser. Try Chrome, Safari, or Edge for the best experience.
            </span>
          </div>
        </CardContent>
      </Card>
    ...
    <div className={`space-y-4 ${className}`}>
        ...
        <Card className="bg-red-50 border-red-200">
          <CardContent className="p-4">
            <div className="space-y-3">
              ...
              {/* Permission denied - show retry options */}
                ...
                <div className="flex flex-col gap-2">
                  ...
                  {isMobile && (
                    ...
                    >
                      ...
                      {(window as any).Capacitor ? 'Open Settings' : 'How to Enable'}
                    </Button>
                  )}
                </div>
              )}
        ...
        <Card className="bg-white border-gray-200 shadow-sm">
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              ...
              <p className="text-gray-800 font-medium">{nlpResult.interpretedAs}</p>
            </div>
            ...
            {nlpResult.filters.length > 0 ? (
              ...
              <div className="text-center py-4">
                <p className="text-gray-500">No specific filters detected in your voice command.</p>
                <Button
                  variant="outline"
                  onClick={reset}
      ...
      {state === 'idle' && (
        <div className="text-center">
          ...
          {isMobile && permissionState === 'unknown' && (
