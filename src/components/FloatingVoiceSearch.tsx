...
export function FloatingVoiceSearch({ onFiltersApplied, onClose, className = '' }: FloatingVoiceSearchProps) {
      ...
      await speechToTextService.startListening(
        ...
        (error: string) => {
          ...
          if (error.includes('permission') || error.includes('denied')) {
            setError('Microphone access is needed for voice search. Please allow microphone access and try again.');
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={reset}>
      <Card className="w-full max-w-md bg-white/95 backdrop-blur-xl border-0 shadow-2xl rounded-3xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
        <CardContent className="p-6">
              {/* Header with State */}
              <div className="text-center mb-6">
                <div className="w-16 h-16 mx-auto mb-4 rounded-full flex items-center justify-center" style={getButtonStyle()}>
                  {getStateIcon()}
                </div>
                ...
                <div className="mb-6 space-y-4">
                  <div className="text-center">
                    <div className="inline-flex items-center gap-2 px-4 py-2 bg-green-100 text-green-800 rounded-full text-sm font-medium">
                      ...
                      {nlpResult.filters.length} filter{nlpResult.filters.length !== 1 ? 's' : ''} found
                    </div>
                  </div>
